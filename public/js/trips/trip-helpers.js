...
export const utils = {
    ...
    getEmployeeTrips: function(employeeId) {
        if (!state._employeeTripsCache.has(employeeId)) {
            // Фильтруем командировки: проверяем как старый формат (employeeId), так и новый (participants)
            const employeeTrips = state.trips.filter(trip => {
                // Старый формат: одиночные командировки с employeeId
                if (trip.employeeId === employeeId) return true;
                
                // Новый формат: групповые командировки с массивом participants
                if (trip.participants && Array.isArray(trip.participants)) {
                    return trip.participants.includes(employeeId);
                }
